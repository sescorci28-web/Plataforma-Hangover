  ...
  if (!myPresence) {
    return (
      <div className="space-y-8 relative">
        ...
        {/* Split grid for Check-In panel & active chats */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-start">
          {/* Check-In Panel */}
          <div>
            <div className="glass-card p-6 md:p-8 border-primary-500/20 bg-gradient-to-tr from-primary-950/20 via-zinc-900/60 to-accent-950/10 rounded-3xl text-center shadow-xl relative overflow-hidden">
              {/* Glow */}
          ...
          <div className="space-y-4">
            ...
            {chats.length === 0 ? (
              <div className="glass-card p-6 bg-zinc-950/20 border-white/5 rounded-2xl text-center space-y-2">
                <MessageSquare className="w-7 h-7 text-zinc-700 mx-auto" />
                <p className="text-[11px] text-zinc-500 leading-normal">
                  No tienes chats activos. Cuando envíes una solicitud y la acepten, la conversación aparecerá aquí permanentemente.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                {chats.map((chat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full glass-card p-3.5 bg-zinc-950/40 border border-white/5 hover:border-white/10 hover:bg-white/2 transition-all rounded-xl text-left flex items-center justify-between gap-3 group cursor-pointer ${
                        activeChat?.id === chat.id ? 'border-primary-500/20 bg-primary-500/3' : ''
                      }`}
                    >
                      ...
                      <MessageSquare className="w-4 h-4 text-zinc-600 group-hover:text-primary-400 transition-colors shrink-0" />
                    </button>
                  );
                })}
              </div>
  ...
  return (
    <div className="space-y-8 relative">
      ...
      {isEditingSocial && (
        ...
        >
          <h4 className="text-xs uppercase font-extrabold tracking-widest text-primary-400">Configurar Perfil Social Social</h4>
      ...
      {/* Main split: Pending requests at the top if any */}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {requests
              ...
              .map((req) => {
                // Find profile of sender in the presenceList
      ...
      {/* Split grid for active users & active chats */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-start">
        {/* Active attendees */}
        <div className="space-y-4">
                          ...
                          <div className="relative shrink-0">
                            <div className="w-11 h-11 rounded-full border border-white/10 bg-zinc-900 overflow-hidden p-0.5">
                              {profile.avatar_url ? (
                                ...
                                <div className="w-full h-full bg-primary-500/10 flex items-center justify-center text-primary-400 font-extrabold text-sm rounded-full">
                                  {profile.full_name?.charAt(0) || "U"}
                                </div>
                              )}
                            </div>
                      ...
                      <div className="mt-4 pt-3.5 border-t border-white/5">
                        {isConnected ? (
                          <button
                            ...
                            className="w-full py-2 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white rounded-xl text-[11px] font-black uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                          >
